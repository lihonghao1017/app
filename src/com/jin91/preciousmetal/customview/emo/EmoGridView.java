...
import com.jin91.preciousmetal.adapter.EmoGridViewAdapter;
import com.jin91.preciousmetal.adapter.ViewPageAdapter;
...
public class EmoGridView extends LinearLayout {
    ...
    private void initViewPage() {
        ...
        _viewPager.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, EmoCommonUtil.getDefaultPannelHeight(_context)));
    ...
    private void initFootDots() {
        viewPager_size = (int) Math.ceil(Emoparser.getInstance(_context.getApplicationContext()).getResIdList().length / pageItemCount);
        int mod = Emoparser.getInstance(_context.getApplicationContext()).getResIdList().length % (Constants.pageSize - 1);
        if (mod == 1) {
            --viewPager_size;
        }
        if (0 < viewPager_size) {
            if (viewPager_size == 1) {
                ...
                for (int i = 0; i < viewPager_size; i++) {
                    ...
                    params.setMargins(5, EmoCommonUtil.getElementSzie(_context) / 2, 5, EmoCommonUtil.getElementSzie(_context) / 2);
    ...
    private int[] getGridViewData(int index) {
        ...
        if (endPos > Emoparser.getInstance(_context.getApplicationContext()).getResIdList().length) {
            endPos = Emoparser.getInstance(_context.getApplicationContext()).getResIdList().length;
        }
        ...
        for (int i = startPos; i < endPos; i++) {
            tmps[num] = Emoparser.getInstance(_context.getApplicationContext()).getResIdList()[i];
            num++;
        }
        if (length > 1) {
    ...
    private GridView getViewPagerItem(final int index) {
        ...
        gridView.setVerticalSpacing(EmoCommonUtil.getElementSzie(_context) / 2 + EmoCommonUtil.getElementSzie(_context) / 3);
        // gridView.setVerticalSpacing(30);
        gridView.setBackgroundColor(Color.TRANSPARENT);
        gridView.setAdapter(new EmoGridViewAdapter(_context, getGridViewData(index)));
    ...
    public void setOnEmoGridViewItemClick(OnEmoGridViewItemClick onFaceGridViewItemClick) {
