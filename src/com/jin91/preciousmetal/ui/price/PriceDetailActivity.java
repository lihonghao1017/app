 ...
 */
public class PriceDetailActivity extends BaseActivity implements RadioGroup.OnCheckedChangeListener, PriceDetailView {
...
//    private boolean kIsSuccess;// 获取k线图是否成功
     ...
     */
    public static void actionLaunch(Context context, String tradeCode, String titleName, String sellPrice, String increase) {
    ...
    @OnClick({R.id.tv_title_back, R.id.tv_title_option, R.id.tv_cycle, R.id.tv_indicator, R.id.rb_nav_locale, R.id.rb_nav_acttrad})
    public void setOnClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.tv_cycle: // 周期
                if (cycleDialog == null) {
                    cycleDialog = new ListViewDialog(mContext, cycleList, new ListViewDialog.ListDialogItemListener() {
            ...
            case R.id.tv_indicator: // 指标
                if (indicatorDialog == null) {
                    indicatorDialog = new ListViewDialog(mContext, indicatorList, new ListViewDialog.ListDialogItemListener() {
            ...
            case R.id.rb_nav_locale: // 本地提醒
                ...
                if (currentPrice != null) {
                    ...
                    localIncrease = MathUtil.keep2Decimal((Double.parseDouble(currentPrice.last) - Double.parseDouble(currentPrice.lastclose)));

                }
                PriceAlertActivity.actionLaunch(mContext, tradeCode, titleName, localSellPrice, localIncrease, 1, 1);
                break;
    ...
    public void onCheckedChanged(RadioGroup group, int checkedId) {
        switch (checkedId) {
            ...
            case R.id.rb_k_line: // k线
                ...
                presenter.getPriceK(TAG, true, tradeCode, cycleList.get(cyclePosition).value);
                break;
        }
    }
    ...
    private void processCycle(int position) {
        ...
        presenter.getPriceK(TAG, true, tradeCode, cycleList.get(position).value);
    }
    ...
    private void processIndicator(int position) {
        ...
        setKChartBottomTitle(bollmaslipcandlestickchart.getSelectedIndex(), indicatorBottom);
    }
    ...
    public void setPrice(Price price) {
        currentPrice = price;
        // 初始化数据
        tvPriceBuy.setText(priceUtil.formatNum(Double.parseDouble(price.buy), price.code)); // 买价
        tvPriceSell.setText(priceUtil.formatNum(Double.parseDouble(price.sell), price.code)); // 卖
        tvPriceMaxvalue.setText(priceUtil.formatNum(Double.parseDouble(price.high), price.code)); // 最高
        tvPriceMinvalue.setText(priceUtil.formatNum(Double.parseDouble(price.low), price.code)); // 最低
        // 涨价
        tvPriceRise.setText(priceUtil.formatNum(Double.parseDouble(price.last) - Double.parseDouble(price.lastclose), price.code));
        double increasePercent = ((Double.parseDouble(price.last) - Double.parseDouble(price.lastclose)) / Double.parseDouble(price.lastclose)) * 100;
        // 涨福
        tvPriceRange.setText(String.format("%.2f", increasePercent) + "%");
        // 开盘价
        tvOpenPrice.setText(Html.fromHtml("开盘: " + "<font color=#F34F4F>" + priceUtil.formatNum(Double.parseDouble(price.open), price.code) + "</font>"));
        // 昨收
        tvYestPrice.setText("昨收: " + priceUtil.formatNum(Double.parseDouble(price.lastclose), price.code));
        // 振幅
        tvPriceShake.setText("振幅: " + priceUtil.getSwing(Double.parseDouble(price.high), Double.parseDouble(price.low), Double.parseDouble(price.open)));
...
//        priceIsSuccess = true;
    ...
    private void setTimeChartData(List<DateValueEntity> dateList) {
        ...
        dayLine.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_white));
        ...
        for (DateValueEntity lineEntity : dateList) {
            DateValueEntity valueEntity = new DateValueEntity(Float.parseFloat(closePrice + ""), lineEntity.getDate());
        ...
        closeLine.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_white));
        closeLine.setLineData(closeDataList);
        lines.add(closeLine);

        PriceDetailK.initSlipLineChartTime(mContext, dateList.size(), slipLineChartTime);
...
//        btn_time.setTextColor(getResources().getColor(R.color.chart_text_c));
    ...
    private void setKChartTopData(List<IStickEntity> ohlcEntities, int indicator) {
        PriceDetailK.initMASlipCandleStickChart(mContext, ohlcEntities.size(), bollmaslipcandlestickchart, macdchart);
        bollmaslipcandlestickchart.setStickData(new ListChartData<IStickEntity>(ohlcEntities));
        List<LineEntity<DateValueEntity>> lines = new ArrayList<LineEntity<DateValueEntity>>();
        ...
        bollM.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_red));
        bollM.setLineData(Utils.getBOLL(ohlcEntities).get(Utils.INDICATOR_BOLL_MID));
        bollLines.add(bollM);

        LineEntity<DateValueEntity> bollU = new LineEntity<DateValueEntity>();
        bollU.setTitle("BOLLU");
        bollU.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_yellow));
        bollU.setLineData(Utils.getBOLL(ohlcEntities).get(Utils.INDICATOR_BOLL_UP));
        bollLines.add(bollU);

        LineEntity<DateValueEntity> bollL = new LineEntity<DateValueEntity>();
        bollL.setTitle("BOLLL");
        bollL.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_purple));
        bollL.setLineData(Utils.getBOLL(ohlcEntities).get(Utils.INDICATOR_BOLL_LOW));
        bollLines.add(bollL);

        bollmaslipcandlestickchart.setBandData(bollLines);
        bollmaslipcandlestickchart.setcurrentSeletedListerne(new seletedIndexListener() {
        ...
        switch (indicator) {
            case INDICATOR_SMA: // 简单移动平均线（SMA）
                ...
                sma5.setLineData(Utils.getSma(ohlcEntities, 5));
                lines.add(sma5);

                // 计算10日均线
                LineEntity<DateValueEntity> sma10 = new LineEntity<DateValueEntity>();
                sma10.setTitle("SMA10");
                sma10.setLineColor(getResources().getColor(R.color.chart_line_yellow));
                sma10.setLineData(Utils.getSma(ohlcEntities, 10));
                lines.add(sma10);
                ...
                sma20.setLineData(Utils.getSma(ohlcEntities, 20));
                lines.add(sma20);

                bollmaslipcandlestickchart.setLinesData(lines);
                bollmaslipcandlestickchart.notifyEvent(bollmaslipcandlestickchart);

                setKChartTopTitle(bollmaslipcandlestickchart.getSelectedIndex(), INDICATOR_SMA);
                break;
            case INDICATOR_BOLL:
                bollmaslipcandlestickchart.setLinesData(bollLines);
                bollmaslipcandlestickchart.notifyEvent(bollmaslipcandlestickchart);

                setKChartTopTitle(bollmaslipcandlestickchart.getSelectedIndex(), INDICATOR_BOLL);
                break;
            case INDICATOR_EMA:

                LineEntity<DateValueEntity> ema12 = new LineEntity<DateValueEntity>();
                ema12.setTitle("EMA12");
                ema12.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_red));
                ...
                ema26.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_yellow));
                ...
                setKChartTopTitle(bollmaslipcandlestickchart.getSelectedIndex(), INDICATOR_EMA);
                break;
        }
        bollmaslipcandlestickchart.addNotify(new ITouchEventResponse() {
            ...
            public void notifyEvent(GridChart chart) {
                ...
                macdchart.setDisplayFrom(maSlipCandleStickChart.getDisplayFrom());
                macdchart.setDisplayNumber(maSlipCandleStickChart.getDisplayNumber());
                macdchart.notifyEvent(maSlipCandleStickChart);

                sliplinechart.setDisplayFrom(maSlipCandleStickChart.getDisplayFrom());
                sliplinechart.setDisplayNumber(maSlipCandleStickChart.getDisplayNumber());
    ...
    private void setKChartTopTitle(int currentIndex, int indicator) {
        setKToptipEmpty();
        IChartData<IStickEntity> iChartData = bollmaslipcandlestickchart.getStickData();
        if (iChartData == null || iChartData.size() == 0) {
            return;
        }
        OHLCEntity ohlcEntity = (OHLCEntity) iChartData.get(currentIndex);

        String datestr = DateUtil.converTime(ohlcEntity.getDate(), "yyyy-MM-dd HH:mm", 0);
        tvKDate.setText(datestr);
        switch (indicator) {
            case INDICATOR_SMA:
                LineEntity<DateValueEntity> sma5 = bollmaslipcandlestickchart.getLinesData().get(0);
                LineEntity<DateValueEntity> sma10 = bollmaslipcandlestickchart.getLinesData().get(1);
                LineEntity<DateValueEntity> sma20 = bollmaslipcandlestickchart.getLinesData().get(2);
                ...
                //###########################
                //sunan,20150128
                tvTipOpen.setText(getString(R.string.price_chart_open) + MathUtil.keep2Decimal(ohlcEntity.getOpen() / AxisCalc));
                tvTipYest.setText(getString(R.string.price_chart_close) + MathUtil.keep2Decimal(ohlcEntity.getClose() / AxisCalc));
                tvTipHigh.setText(getString(R.string.price_chart_high) + MathUtil.keep2Decimal(ohlcEntity.getHigh() / AxisCalc));
                tvTipLow.setText(getString(R.string.price_chart_low) + MathUtil.keep2Decimal(ohlcEntity.getLow() / AxisCalc));
                tvTipInd5.setText(getString(R.string.price_chart_maFirst) + MathUtil.keep2Decimal(maFir.getValue() / AxisCalc));
                tvTipInd10.setText(getString(R.string.price_chart_maMid) + MathUtil.keep2Decimal(maMid.getValue() / AxisCalc));
                tvTipInd20.setText(getString(R.string.price_chart_maLast) + MathUtil.keep2Decimal(maLast.getValue() / AxisCalc));
                //###########################
                if (currentIndex > 0) {
                    OHLCEntity lastOhlcEntity = (OHLCEntity) iChartData.get(currentIndex - 1);
...
//                    tvTipInd5.setTextColor(sma5.getLineColor());
            ...
            case INDICATOR_BOLL:
                ...
                tvTipLow.setText(getString(R.string.price_chart_low) + MathUtil.keep2Decimal(ohlcEntity.getLow() / AxisCalc));
                //#############################
                DateValueEntity ma = bollmaslipcandlestickchart.getLinesData().get(0).getLineData().get(currentIndex);
                DateValueEntity upboll = bollmaslipcandlestickchart.getLinesData().get(1).getLineData().get(currentIndex);
                DateValueEntity dn = bollmaslipcandlestickchart.getLinesData().get(2).getLineData().get(currentIndex);
...
//                tvTipInd20.setTextColor(bollmaslipcandlestickchart.getLinesData().get(2).getLineColor());
            ...
            case INDICATOR_EMA:
                ...
                tvTipLow.setText(getString(R.string.price_chart_low) + MathUtil.keep2Decimal(ohlcEntity.getLow() / AxisCalc));
                //##########################
                DateValueEntity up = bollmaslipcandlestickchart.getLinesData().get(0).getLineData().get(currentIndex);
                DateValueEntity low = bollmaslipcandlestickchart.getLinesData().get(1).getLineData().get(currentIndex);
                ...
                //sunan,20150128
                tvTipInd5.setText(" EMA(12):" + MathUtil.keep2Decimal(up.getValue() / AxisCalc));
                tvTipInd5.setVisibility(View.VISIBLE);
//                tvTipInd5.setTextColor(bollmaslipcandlestickchart.getLinesData().get(0).getLineColor());
                //sunan,20150128
                tvTipInd10.setText(" EMA(26):" + MathUtil.keep2Decimal(low.getValue() / AxisCalc));
                tvTipInd10.setVisibility(View.VISIBLE);
//                tvTipInd10.setTextColor(bollmaslipcandlestickchart.getLinesData().get(1).getLineColor());
    ...
    private void setKChartTopColor(OHLCEntity ohlcEntity, double lastClose) {
        ...
        // 设置标签文本颜色
        if (open == 0) {
            tvTipOpen.setTextColor(PriceDetailK.getColor(mContext, R.color.black));
        } else {
            // 设置标签文本颜色
            tvTipOpen.setTextColor(open != lastClose ? open > lastClose ? PriceDetailK.getColor(mContext, R.color.chart_line_red) : PriceDetailK.getColor(mContext, R.color.chart_line_green) : PriceDetailK.getColor(mContext, R.color.chart_line_gray));
        }

        if (high == 0) {
            tvTipYest.setTextColor(PriceDetailK.getColor(mContext, R.color.black));
        } else {
            // 设置标签文本颜色
            tvTipYest.setTextColor(high != lastClose ? high > lastClose ? PriceDetailK.getColor(mContext, R.color.chart_line_red) : PriceDetailK.getColor(mContext, R.color.chart_line_green) : PriceDetailK.getColor(mContext, R.color.chart_line_gray));
        }

        if (low == 0) {
            tvTipHigh.setTextColor(PriceDetailK.getColor(mContext, R.color.black));
        } else {
            tvTipHigh.setTextColor(low != lastClose ? low > lastClose ? PriceDetailK.getColor(mContext, R.color.chart_line_red) : PriceDetailK.getColor(mContext, R.color.chart_line_green) : PriceDetailK.getColor(mContext, R.color.chart_line_gray));
        }

        if (close == 0) {
            tvTipLow.setTextColor(PriceDetailK.getColor(mContext, R.color.black));
        } else {
            tvTipLow.setTextColor(close != lastClose ? close > lastClose ? PriceDetailK.getColor(mContext, R.color.chart_line_red) : PriceDetailK.getColor(mContext, R.color.chart_line_green) : PriceDetailK.getColor(mContext, R.color.chart_line_gray));
        }
    }
     ...
     * @param indicator
     */
    private void setKChartBottomData(List<IStickEntity> ohlcEntities, int indicator) {
        ...
        switch (indicator) {
            case INDICATOR_MACD: // 底下显示macd图
                ...
                macdchart.setStickData(new ListChartData<IStickEntity>(Utils.getMACD(ohlcEntities)));
            ...
            case INDICATOR_KDJ: // kdj随机指标
                ...
                lineK.setTitle("kdjK");
                lineK.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_red));
                ...
                lineD.setTitle("kdjD");
                lineD.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_yellow));
                ...
                lineJ.setTitle("kdjJ");
                lineJ.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_white));
                lineJ.setLineData(kdjJDData);
                lines.add(lineJ);

                PriceDetailK.initSlipLineChartK(mContext, kdjDData.size(), sliplinechart);
            ...
            case INDICATOR_RSI:
                ...
                lineR.setTitle("rsi6");
                lineR.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_red));
                lineR.setLineData(lineData6);
                lines.add(lineR);

                LineEntity<DateValueEntity> lineS = new LineEntity<DateValueEntity>();
                lineS.setTitle("rsi12");
                lineS.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_yellow));
                lineS.setLineData(Utils.getRsi(ohlcEntities, 12));
                lines.add(lineS);

                PriceDetailK.initSlipLineChartK(mContext, lineData6.size(), sliplinechart);
                sliplinechart.setLinesData(lines);
            ...
            case INDICATOR_ADX:
                ...
                lineADX9.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_yellow));
                lineADX9.setLineData(adx9List);
                lines.add(lineADX9);

                PriceDetailK.initSlipLineChartK(mContext, adx9List.size(), sliplinechart);
                sliplinechart.setLinesData(lines);
                sliplinechart.notifyEvent(sliplinechart);
            ...
            case INDICATOR_ATR:
                ...
                lineAtr15.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_yellow));
                lineAtr15.setLineData(atr15List);
                lines.add(lineAtr15);

                PriceDetailK.initSlipLineChartK(mContext, atr15List.size(), sliplinechart);
                sliplinechart.setLinesData(lines);
                sliplinechart.notifyEvent(sliplinechart);
...
//                sliplinechart.setVisibility(View.VISIBLE);
                ...
                lineDmi20.setLineColor(PriceDetailK.getColor(mContext, R.color.chart_line_yellow));
                lineDmi20.setLineData(atr20List);
                lines.add(lineDmi20);

                PriceDetailK.initSlipLineChartK(mContext, atr20List.size(), sliplinechart);
                sliplinechart.setLinesData(lines);
                sliplinechart.notifyEvent(sliplinechart);
    ...
    private void setKChartBottomTitle(int currentIndex, int indicator) {
        ...
        switch (indicator) {
            case INDICATOR_MACD:
                ...
                MACDEntity macdEntity = (MACDEntity) macdchart.getStickData().get(currentIndex);
                double dif = macdEntity.getDiff();
                double dea = macdEntity.getDea();
                double macd = macdEntity.getMacd();
                tvTipBottom1.setText(" MACD(12,26,9):" + MathUtil.keep2Decimal(macd));
            ...
            case INDICATOR_KDJ:
                ...
                DateValueEntity kdjK = sliplinechart.getLinesData().get(0).getLineData().get(currentIndex);
                DateValueEntity kdjD = sliplinechart.getLinesData().get(1).getLineData().get(currentIndex);
                DateValueEntity kdjJ = sliplinechart.getLinesData().get(2).getLineData().get(currentIndex);
                tvTipBottom1.setText(" K:" + MathUtil.keep2Decimal(kdjK.getValue()));
                tvTipBottom1.setTextColor(sliplinechart.getLinesData().get(0).getLineColor());
                tvTipBottom2.setText(" D:" + MathUtil.keep2Decimal(kdjD.getValue()));
                tvTipBottom2.setTextColor(sliplinechart.getLinesData().get(1).getLineColor());
                tvTipBottom3.setText(" J:" + MathUtil.keep2Decimal(kdjJ.getValue()));
                tvTipBottom3.setTextColor(sliplinechart.getLinesData().get(2).getLineColor());
                break;
            case INDICATOR_RSI:
                macdchart.setVisibility(View.GONE);
                sliplinechart.setVisibility(View.VISIBLE);

                DateValueEntity rsi6 = sliplinechart.getLinesData().get(0).getLineData().get(currentIndex);
                DateValueEntity rsi12 = sliplinechart.getLinesData().get(1).getLineData().get(currentIndex);
                tvTipBottom1.setText(" RSI6:" + MathUtil.keep2Decimal(rsi6.getValue()));
                tvTipBottom1.setTextColor(sliplinechart.getLinesData().get(0).getLineColor());
                tvTipBottom2.setText(" RSI12:" + MathUtil.keep2Decimal(rsi12.getValue()));
                tvTipBottom2.setTextColor(sliplinechart.getLinesData().get(1).getLineColor());
                break;
            case INDICATOR_ADX:
                macdchart.setVisibility(View.GONE);
                ...
                LineEntity<DateValueEntity> adx9Datas = sliplinechart.getLinesData().get(0);
                DateValueEntity adx9 = adx9Datas.getLineData().get(currentIndex);
                tvTipBottom1.setText(" ADX9:" + MathUtil.keep2Decimal(adx9.getValue()));
            ...
            case INDICATOR_ATR:
                ...
                LineEntity<DateValueEntity> atr15Line = sliplinechart.getLinesData().get(0);
                DateValueEntity atr15 = atr15Line.getLineData().get(currentIndex);
                tvTipBottom1.setText(" ATR15:" + MathUtil.keep2Decimal(atr15.getValue()));
            ...
            case INDICATOR_DMI:
                ...
                LineEntity<DateValueEntity> dmi20Line = sliplinechart.getLinesData().get(0);
                DateValueEntity dx20 = dmi20Line.getLineData().get(currentIndex);
                tvTipBottom1.setText(" DX20:" + MathUtil.keep2Decimal(dx20.getValue()));
    ...
    private void setTimeTitle(int index) {
        LineEntity<DateValueEntity> line = slipLineChartTime.getLinesData().get(0);
        ...
        if (date != 0) {
            String datestr = DateUtil.converTime(valueEntity.getDate(), "yyyy-MM-dd HH:mm", 0);
      ...
      /*  tvTimeDate.setTextColor(PriceDetailK.getColor(mContext, R.color.chart_line_white));
        tvTimeTopRun.setTextColor(PriceDetailK.getColor(mContext, R.color.chart_line_white));*/
    }
