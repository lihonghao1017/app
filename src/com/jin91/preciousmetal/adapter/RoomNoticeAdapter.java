...
public class RoomNoticeAdapter extends BaseAdapter {
    ...
    private List<RoomNotice> mList;

    public RoomNoticeAdapter(Context mContext, List<RoomNotice> mList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(mContext).inflate(R.layout.roomnotice_list_item, parent, false);
        ...
        RoomNotice notice = mList.get(position);
        holder.tv_theme_name.setText(notice.Title);
        holder.tv_notice_time.setText(StringUtil.getSectionTime(notice.CreatedTime));
        setTextCollapsb(holder, notice);
        if (notice.Imgs != null && notice.Imgs.size() > 0) {
            holder.gv_them_imgs.setVisibility(View.VISIBLE);
            GrideViewImgAdapter adapter = new GrideViewImgAdapter(mContext, notice.Imgs);
    ...
    public void setTextCollapsb(final ViewHolder holder, RoomNotice message) {
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.tv_theme_contentmore:
                    RoomNotice message = mList.get(position);
